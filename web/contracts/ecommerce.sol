...
contract Ecommerce {
    ...
    constructor(
        address payable _seller //buyer is the deployer
    ) public {
        buyer = msg.sender;
        seller = _seller;
    ...
    function proceedToCheckout() public {
    require(current == state.AWAITING_DELIVERY, "Checkout can only proceed when awaiting delivery");
    require(buyerOK == true && sellerOK == true, "Both buyer and seller must confirm delivery before proceeding with checkout");

    // Transfer funds to the seller
    seller.transfer(address(this).balance);

    // Update contract state to indicate completion
    ...
    function escrow_balance()
        ...
        view
        returns (
            uint //returns the balance of the contract
        )
    {
    ...
    function b_delivery_received() public payable {
        ...
        if (sellerOK == true) release_fund();
    }

    function release_fund() private {
        if (buyerOK && sellerOK) seller.transfer((address(this).balance));
        current = state.COMPLETE;
    }
    function withdraw_amount() public {
        ...
        if (buyerOK == false && sellerOK == true)
            ...
            buyerOK && !sellerOK && end > start + 172800
